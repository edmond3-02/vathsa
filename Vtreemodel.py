...
class VTreeModel(QAbstractItemModel):
	...
	def data(self, index: QModelIndex, role: int = None):
		...
		item: VTreeItem = self.get_item(index)
	...
	def get_item(self, index: QModelIndex = QModelIndex()) -> VTreeItem:
		if index.isValid():
			item: VTreeItem = index.internalPointer()
			if item:
				return item
	...
	def index(self, row: int, column: int, parent: QModelIndex = QModelIndex()) -> QModelIndex:
		...
		parent_item: VTreeItem = self.get_item(parent)
		if not parent_item:
			return QModelIndex()

		child_item: VTreeItem = parent_item.child(row)
		if child_item:
	...
	def parent(self, index: QModelIndex = QModelIndex()) -> QModelIndex:
		...
		child_item: VTreeItem = self.get_item(index)
		if child_item:
			parent_item: VTreeItem = child_item.parent()
		else:
			parent_item = None
	...
	def setup_model_data(self, lines: list, parent: VTreeItem):
		parents = [parent]
		indentations = [0]
